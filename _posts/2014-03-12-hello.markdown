...
The block you pass to `finally` cannot take any arguments and can only return void.
...
self.kitten.then(^(UIImage *image){
    ...
    // provided the above catch did not throw or return a
    // `NSError`: always executes
})
{% endhighlight %}
